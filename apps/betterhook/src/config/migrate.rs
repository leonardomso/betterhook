//! Convert a `lefthook.yml` into betterhook's canonical config.
//!
//! This is a best-effort migrator, not a bug-compatible emulator —
//! we deliberately drop lefthook features whose behavior we're
//! improving (stale priority handling, partial env semantics, etc.)
//! and list every change in a generated `BETTERHOOK_MIGRATION_NOTES.md`
...
/// Minimal deserialization of a single lefthook hook block.
/// We parse the top-level YAML as a free-form Mapping first and only
/// attempt to deserialize values into this struct for keys that look
/// like real git hooks — that way top-level keys like `extends:`,
/// `remotes:`, `templates:` round-trip into migration notes without
/// tripping strict deserialization.
...
/// Summary of a single conversion. Held alongside the converted
/// `RawConfig` so the CLI can write it as `BETTERHOOK_MIGRATION_NOTES.md`.
...
fn convert_command(
    ...
    // Lefthook uses {staged_files} / {all_files} / {push_files} —
    // identical to ours, so the run string is usually untouched.
    // Lefthook sometimes uses {files} for the glob-filtered set; we
    // also use {files}, so again untouched.

    // If the command was declared with an empty run, warn.
    if run.is_empty() {
        report.note(format!(
    ...
    // skip / only in lefthook can be boolean, string, or a list of
    // YAML blocks. We coerce to an Option<String> — a richer mapping
    // is a follow-up.
...
/// Convenience wrapper: read a file, convert, return the tuple.
...
"#;
    ...
    fn sample_round_trips_through_lower() {
        let (raw, report) = from_lefthook_yaml(SAMPLE).unwrap();
...
pre-commit:
  commands:
    a:
      run: "true"
"#;
        let (raw, report) = from_lefthook_yaml(doc).unwrap();
        let cfg = raw.lower().unwrap();
